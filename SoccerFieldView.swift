...
//  QDSoccer
//
//  Created by 锦建杨 on 17/5/8.
//  Copyright © 2017年 Yang Jinjian. All rights reserved.
//
...
class SoccerFieldView: UIView {
    ...
    convenience init(homeFormation: String, guestFormation: String) {
        ...
        self.frame = CGRect(x: 0, y: 0, width: SCREEN_SIZE.width, height: SCREEN_SIZE.width * FIELD_DEFAULT_LENGTH / FIELD_DEFAULT_WIDTH)
    }
    ...
    override func draw(_ rect: CGRect) {
        ...
        //准备坐标参数
        let fieldWidth = SCREEN_SIZE.width - FIELD_LEFT_SPACE * 2                   //实际场地宽度
        ...
        context?.addArc(center: cornerPoint1, radius: 15, startAngle: 0, endAngle: 0.5 * CGFloat(M_PI), clockwise: false)
        context?.drawPath(using: .stroke)
        context?.addArc(center: cornerPoint2, radius: 15, startAngle: 0.5 * CGFloat(M_PI), endAngle: CGFloat(M_PI), clockwise: false)
        context?.drawPath(using: .stroke)
        context?.addArc(center: cornerPoint3, radius: 15, startAngle: CGFloat(M_PI), endAngle: 1.5 * CGFloat(M_PI), clockwise: false)
        context?.drawPath(using: .stroke)
        context?.addArc(center: cornerPoint4, radius: 15, startAngle: 1.5 * CGFloat(M_PI), endAngle: 0, clockwise: false)
        context?.drawPath(using: .stroke)
        //画中线
        ...
        context?.addArc(center: centerPoint, radius: scale * CENTER_CIRCLE_DEFAULT_RADIUS, startAngle: 0, endAngle: 2 * CGFloat(M_PI), clockwise: false)
        context?.drawPath(using: .stroke)
        //画发球点
        context?.addArc(center: centerPoint, radius: 2, startAngle: 0, endAngle: 2 * CGFloat(M_PI), clockwise: false)
        context?.drawPath(using: .stroke)
        //画上方禁区和球门
        ...
        context?.addArc(center: upPenaltyMarkPoint, radius: 1.5, startAngle: 0, endAngle: 2 * CGFloat(M_PI), clockwise: false)
        context?.drawPath(using: .fillStroke)
        //画上方罚球弧
        context?.addArc(center: upPenaltyMarkPoint, radius: scale * PENALTY_ARC_DEFAULT_RADIUS, startAngle: 0.2 * CGFloat(M_PI), endAngle: 0.8 * CGFloat(M_PI), clockwise: false)
        context?.drawPath(using: .stroke)
        //画下方禁区和球门
        ...
        context?.addArc(center: downPenaltyMarkPoint, radius: 1.5, startAngle: 0, endAngle: 2 * CGFloat(M_PI), clockwise: false)
        context?.drawPath(using: .fillStroke)
        //画下方罚球弧
        context?.addArc(center: downPenaltyMarkPoint, radius: scale * PENALTY_ARC_DEFAULT_RADIUS, startAngle: 1.2 * CGFloat(M_PI), endAngle: 1.8 * CGFloat(M_PI), clockwise: false)
        context?.drawPath(using: .stroke)
    }
